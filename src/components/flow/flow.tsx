...
const SankeyDiagram = () => {
  ...
  const renderSankey = (selectedNode: string | null, selectedLink: number | null) => {
    ...
    const columnHeights = uniqueColumns.map((col) => {
      ...
      const totalNodeHeight = columnNodes.reduce(
    ...
    const globalScalingFactor = Math.min(
      dimensions.height / maxColumnHeight,
      1 // Prevent scaling above 1 if columns already fit
    );
    ...
    const categories = [...new Set(updatedNodes.map((node) => {
      if (node.name.includes(".")) {
        return node.name.split(".")[0].split("-")[1];
      }
    }))].filter((category) => category !== undefined);

    const accounts = [...new Set(updatedNodes.map((node) => {
      if (!node.name.includes("-")) {
        return node.name;
      }
    }))].filter((account) => account !== undefined);

    const colorFromNode = (node: FlowNode) => {
      let category = "";
      if (node.name.includes(".")) {
    ...
    svg
      ...
      .attr("stroke-opacity", (d) => selectedLink === d.index ? 1 :
        selectedNode === d.source.name || selectedNode === d.target.name ? 1 :
          0.5)
