import {
  ...
  toFont,
  valueOrDefault
} from "./chunk-SCII3IGZ.js";
import "./chunk-DC5AMYBS.js";

// node_modules/.deno/chartjs-chart-sankey@0.14.0/node_modules/chartjs-chart-sankey/dist/chartjs-chart-sankey.esm.js
var defined2 = (x) => x !== void 0;
...
var setPriorities = (nodes, priority) => {
  if (!priority) return;
  for (const node of nodes.values()) {
...
var getParsedData = (data, parsing) => {
...
var getAllKeysForward = (nodes, visited = /* @__PURE__ */ new Set()) => {
  const keys = [];
...
var nextColumn = (dataWithoutDirectLoops, remainingKeys) => {
...
var flowByNodeCount = (prop) => (a, b) => nodeCount(a.node[prop], prop) - nodeCount(b.node[prop], prop) || a.node[prop].length - b.node[prop].length;
...
function processRest(nodeArray, maxX) {
  ...
  const leftToDo = leftNodes.filter((node) => !defined2(node.y));
  const rightToDo = rightNodes.filter((node) => !defined2(node.y));
  const centerToDo = nodeArray.filter((node) => node.x > 0 && node.x < maxX && !defined2(node.y));
  ...
  centerToDo.forEach((node) => {
    let y = nodeArray.filter((n) => n.x === node.x && defined2(n.y)).reduce((acc, cur) => Math.max(acc, cur.y + Math.max(cur.in, cur.out)), 0);
    y = setOrGetY(node, y);
...
var findStartNode = (nodeArray, maxX) => {
  const size = [
    ...nodeArray
...
function addPadding(nodeArray, padding) {
  let maxY = 0;
  const columnXs = /* @__PURE__ */ new Map();
  const grid = [];
  const getColIndex = (x) => {
    if (!columnXs.has(x)) {
...
var SankeyController = class extends DatasetController {
  ...
  _drawLabels() {
    ...
    const nodes = this._nodes || /* @__PURE__ */ new Map();
    ...
    for (const node of nodes.values()) {
      ...
      const label = (labels == null ? void 0 : labels[node.key]) ?? node.key;
      let textX = x;
...
SankeyController.defaults = {
  ...
  animations: {
    ...
    progress: {
      easing: "linear",
      duration: (ctx) => ctx.type === "data" ? (ctx.parsed._custom.x - ctx.parsed.x) * 200 : void 0,
      delay: (ctx) => ctx.type === "data" ? ctx.parsed.x * 500 + ctx.dataIndex * 20 : void 0
    },
    colors: {
      type: "color",
      properties: [
...
var controlPoints = (x, y, x2, y2) => x < x2 ? {
  cp1: {
    x: x + (x2 - x) / 3 * 2,
    y
  },
...
var applyAlpha = (original, alpha) => color(original).alpha(alpha).rgbString();
var getColorOption = (option, alpha) => typeof option === "string" ? applyAlpha(option, alpha) : option;
...
var Flow = class extends Element {
  draw(ctx) {
...
chartjs-chart-sankey/dist/chartjs-chart-sankey.esm.js:
  (*!
   ...
   *)
*/
//# sourceMappingURL=chartjs-chart-sankey.js.map
